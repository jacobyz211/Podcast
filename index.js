...
function parseRssToShow(xml) {
  ...
  return { title: cleanText(showTitle), artist: cleanText(showArtist), artworkURL: showArt || null, description: cleanText(showDesc).slice(0, 300), tracks: tracks };
}

// ─── feedUrl cache (collectionId → feedUrl) ───────────────────────────────
// Populated during search so /playlist/:id can use rss_ IDs from feedUrl
...
function buildConfigPage(baseUrl) {
  ...
  h += 'input{width:100%;background:#0c0c0f;border:1px solid #222;border-radius:10px;color:#e8e8e8;font-size:14px;padding:12px 14px;margin-bottom:6px;outline:none;transition:border-color .15s}';
  h += 'input:focus{border-color:#4a9eff}input::placeholder{color:#333}';
  ...
  h += '<div class="tip"><b>Save your URL</b> &mdash; copy it to Notes or a bookmark. If the server restarts, paste it below to restore access.</div>';
  h += '<p class="sub">Streams any podcast from Apple Podcasts or RSS feeds. Episodes, shows, creators, and playlists &mdash; all browsable in Eclipse.</p>';
  h += '<div class="pills"><span class="pill">Episodes &amp; shows</span><span class="pill">Creator pages</span><span class="pill">Playlists</span><span class="pill g">RSS feed support</span><span class="pill g">Offline download</span></div>';
  h += '<div class="lbl">Generate Your Addon URL</div>';
  ...
  h += '<div class="hint">Same URL, same library &mdash; nothing breaks.</div>';
  ...
  h += '<div class="step"><div class="sn">4</div><div class="st">Use the <b>Podcast Importer</b> below to save any show to your Eclipse Library</div></div>';
  h += '</div>';
  h += '<div class="warn">Your URL is saved to Redis and survives restarts. Never reinstall the addon after updates.</div>';
  h += '</div>';

  h += '<div class="card">';
  h += '<span class="badge">Podcast Importer</span>';
  h += '<h2>Import a Podcast to Eclipse Library</h2>';
  h += '<p class="sub">Downloads a CSV you can import in Eclipse via Library &rarr; Import CSV.</p>';
  ...
  h += '<footer>Eclipse Podcast Addon v1.1.0 &bull; <a href="' + baseUrl + '/health" target="_blank" style="color:#333;text-decoration:none">' + baseUrl + '</a></footer>';
  ...
  h += 'function doImport(){var btn=document.getElementById("impBtn"),raw=document.getElementById("impToken").value.trim(),purl=document.getElementById("impUrl").value.trim(),st=document.getElementById("impStatus"),pv=document.getElementById("impPreview");if(!raw){st.className="status err";st.textContent="Paste your addon URL first.";return;}if(!purl){st.className="status err";st.textContent="Paste a podcast URL.";return;}var tok=getTok(raw);if(!tok){st.className="status err";st.textContent="Could not find your token in the URL.";return;}btn.disabled=true;btn.textContent="Fetching...";st.className="status";st.textContent="Fetching episodes...";pv.style.display="none";fetch("/u/"+tok+"/import?url="+encodeURIComponent(purl)).then(function(r){if(!r.ok)return r.json().then(function(e){throw new Error(e.error||"Server error "+r.status);});return r.json();}).then(function(data){var tracks=data.tracks||[];if(!tracks.length)throw new Error("No episodes found.");var rows=tracks.slice(0,50).map(function(t,i){return\'<div class="tr"><span class="tn">\'+(i+1)+\'</span><div class="ti"><div class="tt">\'+hesc(t.title)+\'</div><div class="ta">\'+hesc(t.artist)+\'</div></div></div>\';}).join("");if(tracks.length>50)rows+=\'<div class="tr" style="text-align:center;color:#555">+\'+(tracks.length-50)+\' more</div>\';pv.innerHTML=rows;pv.style.display="block";st.className="status ok";st.textContent="Found "+tracks.length+" episodes in \\""+data.title+"\\"";var lines=["Title,Artist,Album,Duration"];tracks.forEach(function(t){function ce(s){s=String(s||"");if(s.indexOf(",")!==-1||s.indexOf("\\"")!==-1){s=\'"\'+s.replace(/"/g,\'""\')+\'"\'}return s;}lines.push(ce(t.title)+","+ce(t.artist)+","+ce(data.title||"")+","+ce(t.duration||""));});var blob=new Blob([lines.join("\\n")],{type:"text/csv"});var a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=(data.title||"podcast").replace(/[^a-zA-Z0-9 _-]/g,"").trim()+".csv";document.body.appendChild(a);a.click();document.body.removeChild(a);btn.disabled=false;btn.textContent="Fetch & Download CSV";}).catch(function(e){st.className="status err";st.textContent=e.message;btn.disabled=false;btn.textContent="Fetch & Download CSV";});}';
  h += '<\/script></body></html>';
  return h;
}
...
app.get('/u/:token/manifest.json', tokenMiddleware, function (req, res) {
  res.json({
    ...
    name: 'Podcasts',
    version: '1.1.0',
    description: 'Search and stream any podcast. Episodes, shows, creators, and playlists.',
    ...
    // "catalog" enables the /album, /artist, /playlist detail endpoints
    // "playlist" in types enables the Playlists filter tab in search
...
app.get('/u/:token/search', tokenMiddleware, async function (req, res) {
  ...
  try {
    var results = await Promise.all([
      ...
      // Shows (albums + source for artists + source for playlists)
      itunesGet('/search', { term: q, media: 'podcast', entity: 'podcast', limit: 15, explicit: 'Yes' })
    ]);
    ...
    // ── Albums (shows) ────────────────────────────────────────────────────
    var showResults = (results[1] && results[1].results) || [];
    var albums = showResults
      .filter(function (s) { return s.kind === 'podcast' || s.wrapperType === 'collection'; })
      .map(mapShow);

    // ── Artists ───────────────────────────────────────────────────────────
    // Derived from show results — deduplicate by artistId, use show artwork.
    // This is more reliable than the iTunes podcastAuthor entity (often empty).
    var artistMap = new Map();
    showResults.forEach(function (p) {
      if (!p.artistId) return;
      if (!artistMap.has(p.artistId)) {
        artistMap.set(p.artistId, {
          id: 'artist_' + String(p.artistId),
          name: cleanText(p.artistName),
          // Use the show's artwork as the artist image — iTunes has no separate artist art for podcasts
          artworkURL: artworkHd(p.artworkUrl600 || p.artworkUrl100),
          genres: p.primaryGenreName ? [p.primaryGenreName] : []
        });
      }
    });
    var artists = Array.from(artistMap.values()).slice(0, 6);

    // ── Playlists ─────────────────────────────────────────────────────────
    // Each podcast that has a feedUrl becomes a browsable playlist via RSS.
    // The ID is "rss_<base64url(feedUrl)>" — handled by /playlist/:id below.
    var playlists = showResults
      .filter(function (p) { return p.feedUrl; })
      .slice(0, 8)
      .map(function (p) {
        var feedId = 'rss_' + Buffer.from(p.feedUrl).toString('base64url');
        // Cache feedUrl so /playlist/:id can find it instantly
        FEED_URL_CACHE.set(feedId, p.feedUrl);
        return {
          id: feedId,
          title: cleanText(p.collectionName),
          creator: cleanText(p.artistName),
          artworkURL: artworkHd(p.artworkUrl600 || p.artworkUrl100),
...
app.get('/u/:token/stream/:id', tokenMiddleware, async function (req, res) {
  ...
  // RSS episode — streamURL already cached
  if (id.startsWith('rss_ep_')) {
    var cached = EPISODE_CACHE.get(id);
    if (cached && cached.streamURL) return res.json({ url: cached.streamURL, format: cached.format || 'mp3' });
    return res.status(404).json({ error: 'RSS episode not found in cache: ' + id });
  }

  // iTunes episode
  if (id.startsWith('ep_')) {
    var epCached = EPISODE_CACHE.get(id);
    if (epCached && epCached.streamURL) return res.json({ url: epCached.streamURL, format: epCached.format || 'mp3' });
    ...
    if (data && data.results) {
      ...
      if (ep) { var mapped = mapEpisode(ep); return res.json({ url: mapped.streamURL, format: mapped.format }); }
    }
  }
...
app.get('/u/:token/album/:id', tokenMiddleware, async function (req, res) {
  ...
  try {
    ...
    if (!data || !data.results || data.results.length === 0) return res.status(404).json({ error: 'Show not found.' });
    ...
    res.json({
      id: rawId, title: cleanText(show.collectionName), artist: cleanText(show.artistName),
      artworkURL: artworkHd(show.artworkUrl600 || show.artworkUrl100),
      year: show.releaseDate ? String(show.releaseDate).slice(0, 4) : null,
      description: show.description || null, trackCount: tracks.length, tracks: tracks
    });
  } catch (e) { console.error('[album] ' + e.message); res.status(500).json({ error: 'Show fetch failed.' }); }
});
...
app.get('/u/:token/artist/:id', tokenMiddleware, async function (req, res) {
  ...
  try {
    ...
    if (!data || !data.results || data.results.length === 0) return res.status(404).json({ error: 'Artist not found.' });
    var artistEntry = data.results.find(function (r) { return r.wrapperType === 'artist'; });
    var podcasts = data.results.filter(function (r) { return r.wrapperType === 'collection' || r.kind === 'podcast'; });
    var name = artistEntry ? cleanText(artistEntry.artistName) : (podcasts[0] ? cleanText(podcasts[0].artistName) : 'Unknown');
    var art  = podcasts[0] ? artworkHd(podcasts[0].artworkUrl600 || podcasts[0].artworkUrl100) : null;
    // Most recent episodes across the creator's shows as topTracks
    var topTracks = [];
    if (podcasts.length > 0 && podcasts[0].collectionId) {
      try {
        var epData = await itunesGet('/lookup', { id: podcasts[0].collectionId, entity: 'podcastEpisode', limit: 10 });
        topTracks = ((epData && epData.results) || [])
          .filter(function (r) { return r.kind === 'podcast-episode' && r.episodeUrl; })
          .map(mapEpisode);
      } catch (e2) { /* skip */ }
    }
    res.json({
      id: rawId, name: name, artworkURL: art,
      bio: null,
      genres: (artistEntry && artistEntry.primaryGenreName) ? [artistEntry.primaryGenreName]
              : (podcasts[0] && podcasts[0].primaryGenreName) ? [podcasts[0].primaryGenreName] : [],
      topTracks: topTracks,
...
app.get('/u/:token/playlist/:id', tokenMiddleware, async function (req, res) {
  ...
  // Decode feedUrl — support both cache lookup and inline base64url
  var feedUrl = FEED_URL_CACHE.get(rawId) || null;
  if (!feedUrl) {
  ...
  if (!xml) {
    // Fallback: try iTunes lookup by feedUrl to get collectionId, then use /album/:id logic
    return res.status(404).json({ error: 'Could not fetch RSS feed. Try opening the show directly.' });
  }
...
// ─── Import (Apple Podcasts URL or RSS feed) ──────────────────────────────
app.get('/u/:token/import', tokenMiddleware, async function (req, res) {
  ...
  if (appleMatch) {
    ...
    var show = data.results.find(function (r) { return r.wrapperType === 'collection'; }) || data.results[0];
    var episodes = data.results.filter(function (r) { return r.kind === 'podcast-episode' && r.episodeUrl; });
    return res.json({ title: cleanText(show.collectionName || show.trackName || 'Podcast'), artworkURL: artworkHd(show.artworkUrl600), tracks: episodes.map(mapEpisode) });
  }
...
app.get('/health', function (req, res) {
  res.json({ status: 'ok', version: '1.1.0', redisConnected: !!(redis && redis.status === 'ready'), activeTokens: TOKEN_CACHE.size, cachedEpisodes: EPISODE_CACHE.size, cachedFeeds: FEED_URL_CACHE.size, timestamp: new Date().toISOString() });
});

app.listen(PORT, function () { console.log('Eclipse Podcast Addon v1.1.0 on port ' + PORT); });
