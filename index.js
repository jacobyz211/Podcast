...
// ─── Config page (EXACT same h+= pattern as working SC addon) ─────────────
function buildConfigPage(baseUrl) {
  ...
  h += '<p class="sub">Streams any podcast from Apple Podcasts or RSS feeds. Episodes, shows, and creators &mdash; all browsable in Eclipse.</p>';
  h += '<div class="pills"><span class="pill">Episodes &amp; shows</span><span class="pill">Creator pages</span><span class="pill g">RSS feed support</span><span class="pill g">Offline download</span></div>';
  ...
  h += '<footer>Eclipse Podcast Addon v1.0.0 &bull; <a href="' + baseUrl + '/health" target="_blank" style="color:#333;text-decoration:none">' + baseUrl + '</a></footer>';

  // ── Client JS (exact same h+= pattern as working SC addon) ──────────────
  h += '<script>';
  h += 'var _gu="",_ru="";';
...
app.get('/u/:token/search', tokenMiddleware, async function (req, res) {
  ...
  try {
    ...
    var artists = ((results[2] && results[2].results) || [])
      ...
      .map(function (a) {
        return { id: 'artist_' + String(a.artistId), name: cleanText(a.artistName), artworkURL: null, genres: a.primaryGenreName ? [a.primaryGenreName] : [] };
      });
...
app.get('/u/:token/artist/:id', tokenMiddleware, async function (req, res) {
  ...
  try {
    ...
    res.json({
      id: rawId, name: name, artworkURL: podcasts[0] ? artworkHd(podcasts[0].artworkUrl600) : null,
      bio: null, genres: (artist && artist.primaryGenreName) ? [artist.primaryGenreName] : [],
      topTracks: [], albums: podcasts.map(mapShow)
    });
...
app.get('/health', function (req, res) {
  res.json({ status: 'ok', version: '1.0.0', redisConnected: !!(redis && redis.status === 'ready'), activeTokens: TOKEN_CACHE.size, cachedEpisodes: EPISODE_CACHE.size, timestamp: new Date().toISOString() });
});

app.listen(PORT, function () { console.log('Eclipse Podcast Addon v1.0.0 on port ' + PORT); });
