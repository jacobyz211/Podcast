...
function detectFormat(url) {
  ...
  if (u.endsWith('.m4a') || u.includes('/m4a/') || u.includes('format=m4a')) return 'm4a';
  if (u.endsWith('.aac')) return 'aac';
  if (u.endsWith('.ogg')) return 'ogg';
  if (u.endsWith('.opus')) return 'ogg';
  return 'mp3';
}
...
function parseRssToShow(xml) {
  ...
  var showTitle = rssTag(chan.replace(/<item[\s\S]*$/i, ''), 'title');
  var showArtist = rssTag(chan, 'itunes:author') || rssTag(chan, 'managingEditor') || '';
  var showArt = rssAttr(chan, 'itunes:image', 'href') || rssAttr(chan, 'image', 'href') || '';
  var showDesc = rssTag(chan, 'description') || rssTag(chan, 'itunes:summary') || '';
  ...
  var tracks = items.map(function (item, i) {
    ...
    var guid = rssTag(item, 'guid') || encUrl || String(i);
    ...
    var obj = {
      ...
      streamURL: encUrl || null,
      format: detectFormat(encUrl || '')
    };
    cacheEpisode(obj);
    return obj;
  }).filter(function (ep) { return !!ep.streamURL; });
...
function buildConfigPage(baseUrl) {
  ...
  h += '<p class="sub">Streams any podcast from Apple Podcasts. Episodes, shows, and creators — all browsable in Eclipse.</p>';
  h += '<div class="pills">';
  h += '<span class="pill">Episodes &amp; shows</span>';
  h += '<span class="pill">Artist/creator pages</span>';
  h += '<span class="pill g">RSS feed import</span>';
  h += '<span class="pill g">Offline download</span>';
  h += '</div>';
  ...
  h += '<div class="hint">Same URL, same library — nothing breaks.</div>';
  ...
  h += '<div class="step"><div class="sn">4</div><div class="st">Use <b>Podcast Importer</b> below to import any podcast into your Eclipse Library</div></div>';
  h += '</div>';
  ...
  h += '<p class="sub">Fetches all episodes as a CSV you can import in Eclipse via Library → Import CSV.</p>';
  ...
  h += 'btn.disabled=true;btn.textContent="Fetching...";st.className="status";st.textContent="Fetching episodes...";pv.style.display="none";';
  h += 'fetch("/u/"+tok+"/import?url="+encodeURIComponent(purl)).then(function(r){if(!r.ok)return r.json().then(function(e){throw new Error(e.error||"Server error "+r.status)});return r.json()}).then(function(data){';
  h += 'var tracks=data.tracks;if(!tracks||!tracks.length)throw new Error("No episodes found.");';
  ...
  h += 'if(tracks.length>50)rows+=\'<div class="tr" style="text-align:center;color:#555">…and \'+(tracks.length-50)+\' more</div>\';';
  ...
  h += 'var lines=["Title,Artist,Album,Duration"];tracks.forEach(function(t){function ce(s){s=String(s||"");if(s.indexOf(",")!==-1||s.indexOf("\\"")!==-1)s=\'\\"\'+s.replace(/\\"/g,\'\\"\\"\')+\'\\"\';return s}lines.push([ce(t.title),ce(t.artist),ce(t.album||data.title),ce(t.duration||"")].join(","))});';
  h += 'var blob=new Blob([lines.join("\\n")],{type:"text/csv"});var a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=(data.title||"podcast").replace(/[^a-zA-Z0-9 \\-]/g,"").trim()+".csv";document.body.appendChild(a);a.click();document.body.removeChild(a);';
...
app.get('/u/:token/search', tokenMiddleware, async function (req, res) {
  ...
  try {
    ...
    var artists = ((results[2] && results[2].results) || [])
      .filter(function (a) { return a.wrapperType === 'artist' || a.artistId; })
      .map(function (a) {
        return {
...
app.get('/u/:token/stream/:id', tokenMiddleware, async function (req, res) {
  ...
  if (id.startsWith('ep_')) {
    ...
    if (data && data.results) {
      ...
      if (ep) {
        cacheEpisode(mapEpisode(ep));
        return res.json({ url: ep.episodeUrl, format: detectFormat(ep.episodeUrl) });
      }
    }
  }
...
app.get('/u/:token/artist/:id', tokenMiddleware, async function (req, res) {
  ...
  try {
    ...
    var firstName = artist ? cleanText(artist.artistName) : (podcasts[0] ? cleanText(podcasts[0].artistName) : 'Unknown');
    var bestArt = podcasts[0] ? artworkHd(podcasts[0].artworkUrl600) : null;
...
// ─── Playlist (RSS feed as playlist) ─────────────────────────────────────
// ID format: rss_{base64url of feed URL}
app.get('/u/:token/playlist/:id', tokenMiddleware, async function (req, res) {
  var rawId = req.params.id;

  if (rawId.startsWith('rss_')) {
...
// ─── Import ───────────────────────────────────────────────────────────────
// Accepts: Apple Podcasts URL (podcasts.apple.com/.../idXXXXX) or RSS feed URL
app.get('/u/:token/import', tokenMiddleware, async function (req, res) {
  ...
  // Apple Podcasts URL → extract numeric ID
  var appleMatch = inputUrl.match(/\/id(\d{6,12})/);
  if (appleMatch || /itunes\.apple\.com|podcasts\.apple\.com/.test(inputUrl)) {
    var podId = appleMatch ? appleMatch[1] : null;
    if (!podId) return res.status(400).json({ error: 'Could not extract podcast ID from Apple Podcasts URL.' });
